...
const Carousel = () => {
    const videos = ["/video/carousel-1.mp4", "/video/carousel-2.mp4", "/video/carousel-3.mp4"];
    ...
    const handleTimeUpdate = () => {
        const currentVideo = videoRefs.current[videoIndex];
        if (currentVideo) {
            const progressPercentage = (currentVideo.currentTime / currentVideo.duration) * 100;
    ...
    return (
        <section className="relative mt-6">
                        ...
                        >
                            <div className="w-full absolute bottom-0 z-50 px-4 py-10">
                                ...
                                <div className="backdrop-blur-sm mb-4 bg-white bg-opacity-20 relative z-10 text-xs text-white py-1.5 px-2 w-fit  rounded-full overflow-hidden">
                                    <span className="relative z-20">0{idx + 1} ~ {videos.length}</span>
                                    <div 
                                        className="absolute top-0 left-0 h-full bg-white bg-opacity-20 transition-all rounded-full"
                                        style={{ width: `${idx === videoIndex ? progress : 0}%`, zIndex: 10 }} 
                                    />
                                </div>
                                <div className="flex w-full justify-between gap-20 items-center">
                                    ...
                                    <ArrowLink href={""} color="purple">Learn More</ArrowLink>
                                </div>
                            </div>
            ...
            <div
                className="hidden absolute flex gap-2 bg-white bg-opacity-10 py-2 px-6 rounded-full left-1/2 transform -translate-x-1/2 bottom-4"
            >
                {videos.map((_, index) => (
                <button
                    key={index}
